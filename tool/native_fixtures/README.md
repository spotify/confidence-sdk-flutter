...
Historical sets are in `android-old` (Android 0.3.2, `a91dd135`) and
`swift-old` (Swift `4be8b1ba`). For old Android, run
`python3 prepare_legacy_android_fixture.py /path/to/android-0.3.2` instead of
copying the driver directly; then use the Gradle command below with Java 17.
This adapts the driver to Mockito, the storage factory, and the older flag
constructor without changing native models. For old Swift, use the same Swift
driver and compiler command against the old source tree. The Swift driver also
generates `calendar.json` with date metadata and a fractional timestamp.
See the [compatibility inventory](../../../../docs/dart-client-provider-compatibility.md).
