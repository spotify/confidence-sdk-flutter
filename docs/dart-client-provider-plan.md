...
The exact new package name is not selected. `confidence_openfeature_provider` is a proposed working name, subject to naming and pub.dev availability checks.
...
- Verify the backend-supported SDK identifier/version and OpenFeature telemetry attribution for this new provider. Do not reuse an Android/Swift identifier by accident.
...
Each step is a small reviewable change or a short sequence of changes. The old bridge can remain in the working tree during development, but the new package must build independently from the first step. A temporary nested package avoids mixing its publication with the existing bridge release automation; final package layout is settled before release.
...
- Check the final dependency graph and published archive for absence of the bridge, its method-channel classes, Confidence native SDK artifacts, and Swift submodule content.
