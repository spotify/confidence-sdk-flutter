...
// Legacy class — kept for existing example app and native bridge consumers.
// New code should use Confidence.builder() or ConfidenceFlutter.create().
class ConfidenceFlutterSdk {
    ...
    for (int i = 0; i < keys.length; i++) {
      ...
      if (flags.containsKey(element)) {
        if (flags[element] is Map<String, dynamic>) {
          ...
          return _parse<T>(flags[element]);
        }
      } else {
        return null;
      }
    }
    return _parse<T>(flags);
  }
  ...
  String getString(String key, String defaultValue) {
    unawaited(
        ConfidenceFlutterSdkPlatform.instance.getString(key, defaultValue));
    return resolveKey(key) ?? defaultValue;
  }
  ...
  Map<String, dynamic> getObject(
    ...
    unawaited(
        ConfidenceFlutterSdkPlatform.instance.getObject(key, defaultValue));
    return resolveKey(key) ?? defaultValue;
  }

  double getDouble(String key, double defaultValue) {
    unawaited(
        ConfidenceFlutterSdkPlatform.instance.getDouble(key, defaultValue));
    return resolveKey<double>(key) ?? defaultValue;
  }
