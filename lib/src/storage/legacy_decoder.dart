...
final class LegacyDecoder {
  ...
  /// Decode UTF-8 per record so damaged bytes cannot erase neighboring events.
  LegacyEventBatch decodeEventBytes(List<int> bytes) {
    final lines = <String>[];
    var start = 0;
    for (var end = 0; end <= bytes.length; end++) {
      if (end != bytes.length && bytes[end] != 10) continue;
      try {
        lines.add(utf8.decode(bytes.sublist(start, end)));
      } on FormatException {
        lines.add('{'); // Invalid record, retaining the physical line position.
      }
      start = end + 1;
    }
    return decodeEvents(lines.join('\n'));
  }
    ...
    try {
      ...
      for (final entry in _list(root['flags'])) {
        ...
        flags[name] = ResolvedFlag(
          ...
          shouldApply: _bool(
            // Android 0.3.x and older Swift snapshots predate this field;
            // their successful evaluations were eligible for apply.
            !flag.containsKey('shouldApply') ||
                    _swift && flag['shouldApply'] == null
                ? true
                : flag['shouldApply'],
          ),
        );
